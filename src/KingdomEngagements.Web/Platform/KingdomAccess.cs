...
public static partial class KingdomIdentity
{
    ...
    {
        var primaryTenant = ParseTenant(principal.FindFirstValue(TenantClaim), TenantClaim);
        var hostTenant = ParseTenant(principal.FindFirstValue("apostolos.tenant_id"), "apostolos.tenant_id");

        if (primaryTenant.HasValue && hostTenant.HasValue && primaryTenant != hostTenant)
            throw new UnauthorizedAccessException("Authenticated identities contain conflicting tenant claims.");

        var resolved = primaryTenant ?? hostTenant;
        if (resolved.HasValue)
            return resolved.Value;

        var headerTenant = ParseTenant(
    ...
    private static Guid? ParseTenant(string? value, string source)
    {
        if (string.IsNullOrWhiteSpace(value))
            return null;

        if (!Guid.TryParse(value, out var tenantId) || tenantId == Guid.Empty)
            throw new UnauthorizedAccessException($"{source} must contain a non-empty UUID.");
