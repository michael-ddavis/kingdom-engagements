...
@Component({
  ...
  template: `
    <section class="eng-page invitations-page">
      ...
      <div class="request-layout">
        <aside class="request-queue" aria-label="Invitation queue">
          @if (loading()) {
            ...
            @for (item of requests(); track item.id) {
              <button type="button" class="request-row" [class.active]="item.id === selectedId()" [attr.aria-pressed]="item.id === selectedId()" (click)="select(item.id)">
                ...
                @if (waitingOnHost(item)) { <b class="waiting">Host</b> } @else { <b>{{ hostReadiness(item) }}%</b> }
              </button>
            }
          }
        </aside>

        <article #detailPanel class="request-detail" tabindex="0" aria-label="Selected invitation details">
          @if (selected(); as item) {
            <header class="detail-heading">
              ...
              <div class="detail-shortcuts">
                <span class="status" [attr.data-status]="item.status">{{ statusLabel(item.status) }}</span>
                @if (reviewable(item)) {
                  <button type="button" class="decision-shortcut" (click)="jumpToDecision()">Review decision ↓</button>
                }
              </div>
            </header>
            ...
            @if (waitingOnHost(item)) {
              ...
              <section class="readiness-strip">
                <div><strong>{{ hostReadiness(item) }}%</strong><small>host information complete</small></div>
                <progress max="100" [value]="hostReadiness(item)" aria-label="Host information complete"></progress>
              </section>

              <div class="readiness-gaps" aria-label="Outstanding host information">
                @if (missingReadiness(item).length) {
                  <span>Still needed</span>
                  @for (gap of missingReadiness(item); track gap.label) {
                    <button type="button" (click)="jumpToSection(gap.section)">{{ gap.label }} ↓</button>
                  }
                } @else {
                  <span>All seven information checks are complete. Approval is a separate decision.</span>
                }
              </div>

              <section class="summary-grid" data-review-section="summary" tabindex="-1" aria-label="Host and event details">
              ...
              <section class="detail-section" data-review-section="ministry" tabindex="-1" aria-label="Requested ministry">
              ...
              <section class="detail-section" data-review-section="terms" tabindex="-1" aria-label="Host commitments">
              ...
              @if (reviewable(item)) {
                <section #reviewPanel class="review-panel" tabindex="-1" aria-label="Invitation decision">
                  <label>
  ...
  styles: [`
    ...
    .readiness-gaps{display:flex;flex-wrap:wrap;gap:.4rem;align-items:center;padding:.7rem 0;font-size:.72rem}
    .readiness-gaps>span{color:var(--eng-muted);margin-right:.25rem}
    .readiness-gaps button{padding:.35rem .5rem;border:1px solid var(--eng-line);border-radius:5px;background:var(--eng-surface);color:var(--action-primary,#172a46);font:inherit;text-align:left;cursor:pointer}
    .readiness-gaps button:focus-visible,[data-review-section]:focus{outline:2px solid var(--action-primary,#172a46);outline-offset:3px}
    .detail-shortcuts{display:grid;justify-items:end;gap:.6rem;flex-shrink:0}
    .decision-shortcut{padding:.5rem .65rem;border:1px solid var(--eng-line);border-radius:7px;background:var(--eng-surface);color:var(--action-primary,#172a46);font:inherit;font-size:.72rem;font-weight:750;cursor:pointer}
    .request-detail:focus-visible,.decision-shortcut:focus-visible{outline:2px solid var(--action-primary,#172a46);outline-offset:3px}
    @media(min-width:981px){
      .page-heading h1{font-size:2rem}
      .page-heading p:last-child{max-width:780px;font-size:.9rem;line-height:1.5}
      .request-layout{height:max(360px,calc(100dvh - 300px));align-items:stretch}
      .request-queue,.request-detail{min-height:0;max-height:100%;box-sizing:border-box;overflow-y:auto;overscroll-behavior-y:contain;scrollbar-gutter:stable}
      .request-queue{align-self:stretch}
      .request-detail{scroll-padding-block:1rem}
    }
  `],
})
export class InvitationsComponent implements OnInit {
  ...
  // Keep these seven checks aligned with SpeakingRequests.Readiness.
  readinessChecks(item: SpeakingRequestDetails): { label: string; section: string; complete: boolean }[] {
    const supplied = (value: string | null | undefined) => !!value?.trim();
    const dated = (value: string) => supplied(value) && !value.startsWith('0001-01-01');
    const determined = (value: string) => supplied(value) && value.toLowerCase() !== 'not-determined';
    const missingEvent = [!dated(item.startDate) && 'start date', !dated(item.endDate) && 'end date', !supplied(item.venueName) && 'venue name'].filter(Boolean).join(', ');
    const missingContact = [!supplied(item.contactName) && 'contact name', !supplied(item.contactEmail) && 'email', !supplied(item.contactPhone) && 'phone'].filter(Boolean).join(', ');
    return [
      { label: missingEvent || 'Event dates and venue', section: 'summary', complete: !missingEvent },
      { label: missingContact || 'Host contact', section: 'summary', complete: !missingContact },
      { label: 'Travel coverage decision', section: 'terms', complete: determined(item.travelCoverageStatus) },
      { label: 'Lodging coverage decision', section: 'terms', complete: determined(item.lodgingCoverageStatus) },
      { label: 'Honorarium decision', section: 'terms', complete: determined(item.honorariumStatus) },
      { label: 'Ministry request', section: 'ministry', complete: supplied(item.ministryRequest) },
      { label: 'Expected attendance', section: 'summary', complete: item.expectedAttendance > 0 },
    ];
  }

  missingReadiness(item: SpeakingRequestDetails) { return this.readinessChecks(item).filter(check => !check.complete); }
  hostReadiness(item: SpeakingRequestDetails): number {
    return Math.round(this.readinessChecks(item).filter(check => check.complete).length * 100 / 7);
  }
  jumpToSection(section: string): void {
    const target = this.detailPanel?.nativeElement.querySelector<HTMLElement>(`[data-review-section="${section}"]`);
    if (!target) return;
  ...
  select(id: string): void {
    ...
    this.api.getRequest(id).subscribe({
      next: item => {
        ...
        this.detailPanel?.nativeElement.scrollTo({ top: 0, behavior: 'instant' });
      },
