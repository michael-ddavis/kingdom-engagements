(() => {
  ...
  const tokenMatch = window.location.pathname.match(/\/invite\/pastor-hickman\/requests\/([^/]+)/);
  ...
  function readiness() {
    ...
    const checks = {
      ...
      travel: input.travelCoverageStatus !== 'not-determined',
      lodging: input.lodgingCoverageStatus !== 'not-determined',
      honorarium: input.honorariumStatus !== 'not-determined',
      ministry: !!input.ministryRequest,
      attendance: input.expectedAttendance > 0
    };
    ...
    document.querySelectorAll('[data-check]').forEach(item =>
      item.classList.toggle('complete', checks[item.dataset.check]));
  }
  ...
  function populate(item) {
    ...
    requestMessage.textContent = item.communications?.at(-1)?.message ||
      'Pastor Hickman’s ministry team requested an update. Review the invitation and send your changes.';
    ...
    document.getElementById('submit-copy').textContent =
      'Every updated field will remain connected to Pastor Hickman’s invitation and preparation workflow.';
  ...
  async function loadUpdate() {
    ...
    formView.hidden = true;
    try {
      const item = await responseJson(await fetch(`/api/public/engagements/hickman-requests/${encodeURIComponent(token)}`));
      populate(item);
  ...
  form.addEventListener('submit', async event => {
    ...
    if (body.endDate < body.startDate)
      return showError('The end date cannot be before the start date.');
    if (token && !value('responseMessage'))
      return showError('Add a response message describing what you updated.');

    submitButton.disabled = true;
    try {
      const endpoint = token
        ? `/api/public/engagements/hickman-requests/${encodeURIComponent(token)}`
        : '/api/public/engagements/hickman-requests';
      const payload = token
        ? { request: body, responseMessage: value('responseMessage') }
        : body;
      const item = await responseJson(await fetch(endpoint, {
  ...
  document.getElementById('submit-another').addEventListener('click', () => {
    window.location.assign('/invite/pastor-hickman');
  });
