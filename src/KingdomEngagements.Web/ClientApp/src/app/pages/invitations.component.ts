...
@Component({
  ...
  template: `
    <section class="eng-page invitations-page">
      <header class="page-heading">
        <div>
          <p class="eng-eyebrow">Invitation review</p>
          <h1>Speaking requests</h1>
          <p>Review host invitations, request missing information, and create assignments without duplicate entry.</p>
        </div>
        <span>{{ requests().length }} requests</span>
      </header>

      @if (message()) {
        <p class="notice" [class.notice--error]="messageError()">{{ message() }}</p>
      }
          ...
          @if (selected(); as item) {
            ...
            @if (reviewable(item)) {
              ...
              <section class="resolved-panel">
                <div><strong>Invitation approved</strong><p>This request is linked to an Engagements assignment.</p></div>
                <button type="button" class="primary" (click)="openAssignment(item.assignmentId)">Open assignment</button>
              </section>
            } @else {
  ...
  `,
  styles: [`
    .invitations-page{padding-top:1.25rem}.page-heading,.detail-heading,.section-heading,.resolved-panel{display:flex;justify-content:space-between;align-items:flex-start;gap:1rem}.page-heading{padding-bottom:1.25rem;border-bottom:1px solid var(--eng-line)}.page-heading h1{margin:0;font-size:clamp(2rem,4vw,3.4rem);letter-spacing:-.04em}.page-heading p:last-child,.detail-heading p:last-child{margin:.5rem 0 0;color:var(--eng-muted)}.page-heading>span,.section-heading>span{padding:.38rem .55rem;border-radius:999px;background:#edf0f4;color:#5f6878;font-size:.7rem;font-weight:800}.request-layout{display:grid;grid-template-columns:minmax(280px,.72fr) minmax(0,1.7fr);gap:1rem;margin-top:1rem}.request-queue,.request-detail{border:1px solid var(--eng-line);border-radius:12px;background:var(--eng-surface)}.request-queue{align-self:start;overflow:hidden}.request-row{position:relative;display:grid;width:100%;gap:.25rem;padding:1rem 3.2rem 1rem 1rem;border:0;border-bottom:1px solid var(--eng-line);text-align:left;background:transparent;color:var(--eng-ink);cursor:pointer}.request-row:last-child{border-bottom:0}.request-row.active{background:#f4f1fb;box-shadow:inset 3px 0 0 var(--eng-violet)}.request-row strong{font-size:.9rem}.request-row small,.request-row em{color:var(--eng-muted);font-size:.7rem;font-style:normal}.request-row b{position:absolute;right:1rem;bottom:1rem}.status{display:inline-flex;width:max-content;padding:.3rem .48rem;border-radius:999px;background:#eef0f4;color:#5e6675;font-size:.62rem;font-weight:900;text-transform:capitalize}.status[data-status='approved']{background:#e8f5ed;color:#257553}.status[data-status='declined']{background:#fbe9e8;color:#9d4541}.status[data-status='information-needed']{background:#fff2d9;color:#8b6617}.request-detail{padding:1.25rem}.detail-heading h2{margin:0;font-size:1.7rem}.readiness-strip{display:grid;grid-template-columns:auto 1fr;align-items:center;gap:1rem;margin-top:1rem;padding:.8rem 0;border-top:1px solid var(--eng-line);border-bottom:1px solid var(--eng-line)}.readiness-strip strong,.readiness-strip small{display:block}.readiness-strip progress{width:100%}.summary-grid,.terms-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem;margin-top:1rem}.summary-grid article,.terms-grid article{padding:.75rem;border:1px solid var(--eng-line);border-radius:8px;background:#fbfaf8}.summary-grid span,.summary-grid small,.terms-grid span{display:block;color:var(--eng-muted);font-size:.63rem}.summary-grid strong,.terms-grid strong{display:block;margin-top:.2rem;font-size:.76rem}.detail-section{margin-top:1rem;padding-top:1rem;border-top:1px solid var(--eng-line)}.detail-section h3{margin:0;font-size:1.05rem}.detail-section>p:last-child{color:#596476;line-height:1.6}.timeline{display:grid;margin-top:.6rem}.timeline article{display:grid;grid-template-columns:14px 1fr;gap:.5rem;padding:.55rem 0}.timeline article>span{width:8px;height:8px;margin-top:.25rem;border:2px solid var(--eng-violet);border-radius:50%}.timeline strong{font-size:.72rem}.timeline p{margin:.18rem 0;color:#5f697a;font-size:.7rem;line-height:1.45}.timeline small{color:#8a919e;font-size:.62rem}.review-panel{margin-top:1rem;padding:1rem;border:1px solid #ddd5f4;border-radius:10px;background:#f8f6fc}.review-panel label{display:grid;gap:.4rem}.review-panel label span{font-size:.72rem;font-weight:800}.review-panel textarea{width:100%;padding:.7rem;border:1px solid #cfd3dc;border-radius:8px;resize:vertical}.review-actions{display:flex;flex-wrap:wrap;gap:.55rem;margin-top:.75rem}.review-actions button,.resolved-panel button{min-height:40px;padding:.55rem .8rem;border-radius:8px;border:1px solid #cfd3dc;font-weight:800;cursor:pointer}.primary{color:#fff;background:#171d2d!important;border-color:#171d2d!important}.secondary{background:#fff}.danger{color:#a03f43;background:#fff;border-color:#e1bbbb!important}.resolved-panel{margin-top:1rem;padding:1rem;border-radius:9px;background:#f2f5f3}.resolved-panel p{margin:.25rem 0 0;color:var(--eng-muted);font-size:.72rem}.notice{margin:1rem 0 0;padding:.75rem 1rem;border-left:3px solid #2b8764;background:#edf8f1;color:#2b6b52}.notice--error{border-color:#a64b4b;background:#fff1f0;color:#94433f}.empty{padding:2rem;color:var(--eng-muted);text-align:center}.review-actions button:disabled{opacity:.55;cursor:wait}
...
export class InvitationsComponent implements OnInit {
  ...
  reload(preferredId?: string): void {
    ...
    this.api.getRequests().subscribe({
      next: requests => {
        ...
        const id = preferredId ?? this.selectedId() ?? requests[0]?.id ?? null;
  ...
  approve(item: SpeakingRequestDetails): void {
    if (!window.confirm('Approve this invitation and create its Engagements assignment?')) return;
    this.saving.set(true);
    this.api.approveRequest(item.id).subscribe({
      next: result => {
        this.saving.set(false);
        this.setMessage('Invitation approved. The assignment was created from the host intake.');
        this.reload(item.id);
