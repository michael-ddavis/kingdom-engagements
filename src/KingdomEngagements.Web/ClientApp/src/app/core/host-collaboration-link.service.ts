...
export class HostCollaborationLinkService {
  ...
  private mountCard(
    ...
    const hostUrl = envelope.termsUrl;
    if (!hostUrl) return;
  ...
  private ensureStyles(): void {
    ...
    style.textContent = `
      ...
      @media(max-width:520px){.apostolos-host-collaboration__actions{display:grid;grid-template-columns:1fr 1fr}.apostolos-host-collaboration__actions button,.apostolos-host-collaboration__actions a{width:100%}.apostolos-host-collaboration__url{max-width:calc(100vw - 60px)}}
    `;
